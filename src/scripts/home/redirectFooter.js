...
export default class RedirectFooter {
  constructor(elm, duration = 5) {
    this.$elms = {
      ...
      svgGradientBottom: elm.querySelector('svg stop#anim-grad-color-active-bottom'),
    };
  ...
  intersectionObserver() {
    const isVisible = (entries) => {
      entries.forEach(entry => {
        ...
        let progress = (intersectionRatio > 0.7 ? 1 : intersectionRatio > 0.3 ? intersectionRatio : 0) * 100;

        gsap.to(this.$elms.svgGradientTop, { attr: { offset: `${progress * 1.36}%` }, duration: 0.3, ease: 'none' });
        gsap.to(this.$elms.svgGradientBottom, { attr: { offset: `${progress}%` }, duration: 0.3, ease: 'none' });
      });
    };

    const threshold = Array.from(Array(20).keys()).map(e => e / 20).filter(e => e > 0.2 && e < 0.8);
