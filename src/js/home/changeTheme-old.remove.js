...
export default class ChangeThemeOldRemove {
  ...
  animation(target, style, className, isAdd = false) {
    ...
    const bgColor = () => {
      if (isAdd) return style.bgColor;

      const isThemes = document.body.classList.contains('theme-dark') && document.body.classList.contains('theme-orange');

      if (!isThemes) return '#fff';

      return this.styles[this.state.in].bgColor;
    }
